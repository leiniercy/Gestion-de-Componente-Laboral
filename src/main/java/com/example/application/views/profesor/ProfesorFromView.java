...
@PageTitle("Profesor")
@Route(value = "profesor-form/:profesorID?/:action?(edit)", layout = MainLayout.class)
@Uses(Icon.class)
@RolesAllowed("admin")
public class ProfesorFromView extends Div implements BeforeEnterObserver {

    private final String PROFESOR_ID = "profesorID";
    private final String PROFESOR_EDIT_ROUTE_TEMPLATE = "profesor-form/%d/edit";
    ...
    public ProfesorFromView(@Autowired ProfesorService profesorService) {
        ...
        save.addClickListener(e -> {
            try {
                ...
                clearForm();
                refreshGrid();
                Notification.show("Profesor details stored.");
                UI.getCurrent().navigate(ProfesorFromView.class);
